...
static unsigned int dT = 22; // 10ms　調整することで途切れが改善した

static SSMApi<rec_sound, rec_sound_property> *RECORD;

int main(int aArgc, char *aArgv[])
{
    SSMApi<rec_sound,rec_sound_property> resound(SOUNDHANDLER_SNAME, 0);
    RECORD = &resound;
    rec_sound *redata = &resound.data;
    ...
    try
    {
        ...
        while (!gShutOff)
        {
            pa_result = pa_simple_read(record_stream, buf , BUF_SIZE, &pa_errno);
            if (pa_result < 0)
            {
...
static void setupSSM(void)
{
    ...
    if (!RECORD->create(5.0, (double)dT / 1000.0))
    {
